...
def main():
    ...
    filename = './QMIX_v10_cnn_lstm_trained_critic.pth'

    gnet.share_memory()         # share the global parameters in multiprocessing
    #opt = SharedAdam(gnet.parameters(), lr=0.00001)      # global optimizer
    opt = QMIXOptimizer(gnet.parameters(), lr=0.00001)      # global optimizer
    ...
    filename = './QMIX_v10_cnn_lstm_trained_critic_actor_1.pth'
    save_checkpoint(filename,gnet,opt)

    with open('QMIX__v10_cnn_lstm_trained_critic_actor.txt', 'a') as f:
        for item in res:
            f.write("%s\n" % item)
